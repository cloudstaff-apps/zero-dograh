...
:class:`GrokRealtimeLLMService`. Closely mirrors the OpenAI Realtime
wrapper because Grok Realtime supports runtime ``session.update`` for
...
- **Function-call deferral** until the bot stops speaking.
- **finalized=True on TranscriptionFrame** for parity with other realtime
  wrappers.
"""
...
class DograhGrokRealtimeLLMService(GrokRealtimeLLMService):
    """Grok Realtime with Dograh engine integration quirks. See module docstring."""
        ...
        try:
            ...
            if function_call_item:
                ...
                if self._bot_is_speaking:
                    ...
                    logger.debug(
                        f"{self}: deferring function call {evt.name} "
